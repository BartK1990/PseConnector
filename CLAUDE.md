...
5. Maps the result into Blazor.Bootstrap `ChartData`/`LineChartDataset`, one dataset per numeric series, cycling colors via `ColorUtility.CategoricalTwelveColors[i % 12]`. Chart is a `<LineChart @ref="_lineChart">`, initialized empty in `OnAfterRenderAsync(firstRender)` and updated via `_lineChart.UpdateAsync(...)` after fetch.
...
7. Register the new route in [NavMenu.razor](PseConnector.Web/Layout/NavMenu.razor).
